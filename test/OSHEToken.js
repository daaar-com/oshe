...
describe("🚩 Full OSHE Token Flow", function () {
    this.timeout(120000);
    ...
    describe("OSHE ERC20", function () {
        // `beforeEach` will run before each test, re-deploying the contract every
        // time. It receives a callback, which can be async.
        beforeEach(async function () {
            [owner, alice] = await ethers.getSigners();
            // deploy contract
            const OSHEToken = await ethers.getContractFactory("OSHEToken");
            myContract = await OSHEToken.connect(owner).deploy();
        ...
        describe("mintNewTokens()", function () {
            it("Should be able to mint new OSHE tokens", async function () {
            ...
            it("Should be able to transfer OSHE tokens", async function () {
            ...
            it("Should be able to burn OSHE tokens", async function () {
                await myContract.mint(alice.address, 10);
